...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, QGroupBox,
                               ...
                               QLineEdit, QSpinBox, QFrame, QRadioButton, QProgressBar)
...
def apply_theme(window, theme_name):
    ...
    for dock in window.docks.values():
        ...
        # 设置Dock的标题栏和标题文字
        # 只有自定义的 DraggableFrame 才有 title_bar 和 title_label 属性
        ...
        for child in dock.findChildren(QWidget):
            # 检查是否是 title_label（如果有该属性）
            is_title_label = hasattr(dock, 'title_label') and child is dock.title_label
            # 检查是否是 close_button（如果有该属性）
            is_close_button = hasattr(dock, 'close_button') and child is dock.close_button

            if isinstance(child, QLabel) and not is_title_label:
                # 排除一些特殊处理的QLabel
            ...
            # QPushButton 在Dock内部的样式 (例如 "执行", "打开仿真模块" 等)
            elif isinstance(child, QPushButton) and not is_close_button:
                # 跳过语义按钮（已有语义颜色的按钮，如开始、停止、删除等）
                if child.objectName() and child.objectName().startswith("semantic_button_"):
                    continue
                ...
                # 同时为tab内的table应用样式
                for table in child.findChildren(QTableWidget):
                    table_style = f"""
                        QTableWidget {{
                            background-color: transparent;
