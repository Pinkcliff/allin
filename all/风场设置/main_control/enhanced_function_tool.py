...
class EnhancedFunctionToolWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        if factory is not None:
            ...
            print(f"[FunctionTool] 函数模块加载成功")
            print(f"[FunctionTool] 可用函数: {len(self.available_functions)} 种")
            print(f"[FunctionTool] 分类: {len(self.categories)} 个")
        else:
            print("警告: 无法导入函数模块")
    ...
    def _apply_function(self):
        ...
        if self.current_function == 'custom_expression' and 'expression' in params:
            try:
                ...
                if CustomExpressionFunction is None or FunctionParams is None:
                    raise ImportError("无法导入CustomExpressionFunction")
                ...
                print(f"[FunctionTool] 自定义表达式验证成功")
            except Exception as e:
